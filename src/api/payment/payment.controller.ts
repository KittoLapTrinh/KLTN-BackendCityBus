...
export class PaymentController {
  ...
  //=================================================================
  @Get('zalopayAdmin/:orderCode/url')
  @HttpCode(HttpStatus.OK)
  @Roles(RoleEnum.STAFF)
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  async getZaloPayPaymentAdminUrl(
    @Param('orderCode') orderCode: string,
    @CurrentUser() user,
  ) {
    return await this.paymentService.getZaloPayPaymentAdminUrl(
      orderCode,
      user.id,
    );
  }

  @Post('zalopayAdmin/check-status')
  @HttpCode(HttpStatus.OK)
  @Roles(RoleEnum.STAFF)
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  async checkStatusZaloPayAdmin(
    @Body() dto: CheckStatusZaloPayPaymentDto,
    @CurrentUser() user,
  ) {
    return await this.paymentService.checkStatusZaloPayAdmin(dto, user.id);
  }
}
